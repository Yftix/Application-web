...
class PdoGsb
{
    ...
    private static $bdd = 'dbname=u295243207_hugomagnant';
    private static $user = 'u295243207_admin';
    private static $mdp = 'Administration1';
     ...
     * 
     * @param String $status status de la recuperation des mois
     *
     *
     * @return un tableau associatif de clé un mois -aaaamm- et de valeurs
     *         l'année et le mois correspondant
     */
    public function getLesMois($status)
    {

        $requetePrepare = PdoGSB::$monPdo->prepare(
            'SELECT DISTINCT fichefrais.mois AS mois FROM fichefrais '
            . 'WHERE idetat= :unStatus '
            . 'ORDER BY fichefrais.mois desc'
    
        );
        $requetePrepare->bindParam(':unStatus', $status, PDO::PARAM_STR);
        $requetePrepare->execute();
        $lesMois = array();
        while ($laLigne = $requetePrepare->fetch()) {
            $mois = $laLigne['mois'];
    ...
    /**
     * Retourne les informations d'une fiche de frais d'un visiteur pour un
     * mois donné et qui est et cours de cloture
     *
     * @param String $idVisiteur ID du visiteur
     * @param String $mois       Mois sous la forme aaaamm
     * @param String $status     status de la fiche de frais a recuperer
     *
     * @return un tableau avec des champs de jointure entre une fiche de frais
     *         et la ligne d'état
     */
    public function getLesInfosFicheFraisStatus($idVisiteur, $mois, $status)
    {
        $requetePrepare = PdoGSB::$monPdo->prepare(
            'SELECT fichefrais.idetat as idEtat, '
            . 'fichefrais.datemodif as dateModif,'
        ...
        $requetePrepare->bindParam(':unIdVisiteur', $idVisiteur, PDO::PARAM_STR);
        $requetePrepare->bindParam(':unMois', $mois, PDO::PARAM_STR);
        $requetePrepare->bindParam(':statut', $status, PDO::PARAM_STR);
        $requetePrepare->execute();
        $laLigne = $requetePrepare->fetch();
        return $laLigne;
    }
    ...
    /**
     * Modifie la date de modification d'une fiche de frais.
     *
     * @param String $idVisiteur ID du visiteur
     * @param String $mois       Mois sous la forme aaaamm
     *
    ...
    /**
     * Met à jour la table ligneFraisHorsForfait en modifant le mois du frais
     *
     * @param String $idVisiteur ID du visiteur
     * @param String $mois       Mois sous la forme aaaamm
     * @param Array  $lesFrais   tableau associatif de clé idFrais et
     *                           de valeur du nouveau libelle du frais
     *
     * @return null
     */
    public function majMoisLibelleFraisHorsForfait($mois, $lesFrais)
    {
        $lesCles = array_keys($lesFrais);
        foreach ($lesCles as $unIdFrais) {
            ...
            $requetePrepare = PdoGSB::$monPdo->prepare(
                ...
                . 'SET lignefraishorsforfait.mois = :unMois, lignefraishorsforfait.libelle = :unLibelle  '
     ...
     /**
     * Retourne sous forme d'un tableau toutes les fiches de frais
     *
     * @return  nom visiteur, prenom visiteur, le mois,
     */
    public function getFicheFraisValidee()
    {
        $requetePrepare = PdoGSB::$monPdo->prepare(
            'SELECT visiteur.prenom as prenom,'
            . 'visiteur.nom as nom,'
